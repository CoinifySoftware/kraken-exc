...
function parseCurrencyPair(pair) {
  function getValidCurrency(sliceLength) {
    ...
    const isSupportedBase = constants.SUPPORTED_BASE_CURRENCIES.includes(convertedBase);
    //We assume that a supported base is a valid currency.
    if (isSupportedBase || convertedBase !== baseCurrency) {
