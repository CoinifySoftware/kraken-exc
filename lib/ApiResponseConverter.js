 ...
 * @returns {object} internal API representation of a trade as defined in the readme
 */
function convertFromKrakenTrade(id, trade) {
  ...
  const baseKrakenCurrency = trade.pair.slice(0, 4);
  const quoteKrakenCurrency = trade.pair.slice(4);
  ...
  return {
    ...
    state: 'closed',
    baseCurrency,
    baseAmount: isBuy ? -baseAmount : baseAmount,
    feeAmount,
    quoteCurrency,
    quoteAmount: isBuy ? -quoteAmount : quoteAmount,
    type: trade.type,
