...
function parseCurrencyPair(pair) {
  // Helper function to try different slice lengths (4 first, then 3)
  ...
  if (!result) {
    throw new Error(`Invalid base currency code in pair: ${pair}`);
  }
  ...
  if (quoteCurrency === remainingPair) {
    throw new Error(`Invalid quote currency code in pair: ${pair}`);
  }
...
function convertFromKrakenTrade(id, trade) {
  ...
  const { baseCurrency, quoteCurrency } = parseCurrencyPair(trade.pair);
